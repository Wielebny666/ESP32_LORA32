...
uint32_t SX1276GetTimeOnAir(RadioModems_t modem, uint8_t pktLen)
{
    ...
    case MODEM_FSK:
    {
        airTime = round((8 * (SX1276.Settings.Fsk.PreambleLen + ((SX1276Read(REG_SYNCCONFIG) & ~RF_SYNCCONFIG_SYNCSIZE_MASK) + 1) + ((SX1276.Settings.Fsk.FixLen == 0x01) ? 0.0 : 1.0) + (((SX1276Read(REG_PACKETCONFIG1) & ~RF_PACKETCONFIG1_ADDRSFILTERING_MASK) != 0x00) ? 1.0 : 0) + pktLen + ((SX1276.Settings.Fsk.CrcOn == 0x01) ? 2.0 : 0)) /
...
void SX1276SetRx(uint32_t timeout)
{
    ...
    if (SX1276.Settings.Modem == MODEM_FSK)
    {
        ...
        {
            TimerSetValue(&RxTimeoutSyncWord, ceil((8.0 * (SX1276.Settings.Fsk.PreambleLen + ((SX1276Read(REG_SYNCCONFIG) & ~RF_SYNCCONFIG_SYNCSIZE_MASK) + 1.0) + 10.0) /
                                                    (double)SX1276.Settings.Fsk.Datarate) *
                                                   1e3) +
                                                  4);
...
void SX1276SetTx(uint32_t timeout)
{
    ...
    case MODEM_FSK:
    {
        ...
        // DIO4=LowBat
        // DIO5=ModeReady
        SX1276Write(REG_DIOMAPPING1, (SX1276Read(REG_DIOMAPPING1) & RF_DIOMAPPING1_DIO0_MASK & RF_DIOMAPPING1_DIO1_MASK & RF_DIOMAPPING1_DIO2_MASK) |
...
void SX1276Write(uint8_t addr, uint8_t data)
{
    ...
    ESP_ERROR_CHECK(spi_device_polling_transmit(SX1276.Spi, &t));
}
