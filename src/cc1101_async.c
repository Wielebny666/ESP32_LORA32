...
void cc1101_rx_async(void *pvParameter)
{
    ...
    set_pa_table(spi);
    set_baud_rate(spi, 3);
    set_magn_target(spi, MAGN_42DB);
    set_filter_length(spi, FILTER_16DB);
    set_rx_channel_bandwidth(spi, 58);
    set_rx_attenuation(spi, ATTENUATION_18dB);
    set_hysteresis_level(spi, HYSTERESIS_LARGE);
    print_config();
    vTaskDelay(200 / portTICK_PERIOD_MS);
    set_idle_state(spi);
